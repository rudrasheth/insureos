...
import { generateEnvironmentSummary } from "../agents/environmentAgent";
 ...
 * Generate a contextual environment profile based on email activity
 * READ-ONLY, deterministic, advisory only
 * Auth: Bearer <session_token>
 */
export const getEnvironmentProfile = httpAction(async (_ctx: any, request: Request) => {
  try {
    // Validate method
    ...
    console.log(`[AgentAPI] Generating environment profile`);

    // Validate session and get user_id
    const userId = await validateSessionAndGetUserId(sessionToken);
    console.log(`[AgentAPI] Session validated for user: ${userId}`);

    // Generate environment summary
    const summary = await generateEnvironmentSummary(userId);
    ...
    if (errorMessage.includes("Invalid session token")) {
      console.error(`[AgentAPI] Invalid session: ${errorMessage}`);
      return new Response(
    ...
    if (errorMessage.includes("Session expired")) {
      console.error(`[AgentAPI] Session expired: ${errorMessage}`);
      return new Response(
    ...
    console.error(`[AgentAPI] Environment profile error: ${errorMessage}`);
    return new Response(
      JSON.stringify({
        error: "Failed to generate environment profile",
