...
import { Send, Bot, User, Sparkles, Loader2, Mail, ShieldAlert, BarChart3, MessageSquare, FileText, Share2 } from 'lucide-react';
...
const AgentInterface = () => {
    ...
    const handleShareWhatsApp = () => {
        const summaryText = messages
            .filter(m => m.role === 'agent' && m.type !== 'error')
            .map(m => m.content)
            .join("\n\n")
            .slice(0, 1000);

        const waUrl = `https://wa.me/?text=${encodeURIComponent(`*InsureOS Report Summary*\n\n${summaryText}\n\n_Generated by Agent Insure_`)}`;
        window.open(waUrl, '_blank');
    };
            ...
            <div className="p-6 border-b border-line bg-canvas/30 flex items-center justify-between">
                ...
                <div className="hidden md:flex gap-2">
                    ...
                    <button
                        ...
                        className="px-3 py-1 rounded-full bg-white border border-line flex items-center gap-2 text-xs font-bold text-green-600 uppercase tracking-widest shadow-sm hover:bg-green-50 hover:text-green-700 hover:border-green-200 transition-all cursor-pointer"
                    >
